  ...
  for (i = 0;
	...
	case STT_OBJECT:
	  ...
	  if (sym->st_name != 0 && sym->st_shndx == 0)
	    {
	      char khook_type;
	      kvaddr_t va = khook_symbol_to_kvaddr(name, NULL, &khook_type);
	      if(!va)
		return grub_error (GRUB_ERR_BAD_MODULE,
				   N_("symbol `%s' not found"), name);
	      sym->st_value = (Elf_Addr) va;
	      if (khook_type=='t' || khook_type=='T')
	...
	case STT_FUNC:
	  ...
	  if (bind != STB_LOCAL)
	    if (grub_dl_register_symbol (name, (void *) sym->st_value, 1, mod))
	      return grub_errno;
#endif
	  if (grub_strcmp (name, "module_init") == 0)
	    mod->init = (void (*) (grub_dl_t)) sym->st_value;
	  else if (grub_strcmp (name, "module_exit") == 0)
...
grub_dl_load_core_noinit (void *addr, grub_size_t size)
{
  ...
  if (e->e_type != ET_REL)
    {
      (void)(grub_error (GRUB_ERR_BAD_MODULE, N_("this ELF file is not of the right type")));
      return 0;
    }
  ...
  if (size < e->e_shoff + e->e_shentsize * e->e_shnum)
    {
      (void)(grub_error (GRUB_ERR_BAD_OS, "ELF sections outside core"));
      return 0;
    }
